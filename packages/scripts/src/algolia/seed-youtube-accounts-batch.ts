...
const CHECKPOINT_FILE = "youtube_accounts_checkpoint.json";
...
const processAccountBatch = async (
  ...
  for (const account of accounts) {
    try {
      ...
      const ytData = account.ytData as YTData | null;
      ...
      algoliaObjects.push({
        ...
        madeForKids: ytData?.status?.madeForKids ?? false,
        videoCount: Number(ytData?.statistics?.videoCount ?? 0),
        bannerURL:
          account.bannerUrl ??
          ytData?.brandingSettings?.image?.bannerExternalUrl ??
          "",
        categories: categorySlugs,
        createdDate: ytData?.snippet?.publishedAt ?? null,
      });
