...
import { useAccountSearch } from "@ratecreator/hooks";
 ...
 * Features include:
 * - Global search with Elasticsearch integration
 ...
 * Features:
 * - Real-time search with Elasticsearch via useAccountSearch hook
 * - Results sorted by the API
 * - Loading states
 * - Error handling
 *
...
const SearchComponent = ({
  ...
  const { results: searchResults, isLoading } = useAccountSearch(searchTerm);
  ...
  useEffect(() => {
    setFilteredResults(
      searchResults.map((hit) => ({
        accountId: hit.objectID || hit.accountId,
...
export const CommandBar: React.FC<{ children: React.ReactNode }> = ({
  ...
  // Keyboard shortcuts — memoized to prevent KBar re-initialization on every render
