...
async function getTwitterMetadata(tweetId: string): Promise<Metadata> {
  try {
    const response = await axios.get(
...
async function getTikTokMetadata(url: string): Promise<Metadata> {
  try {
...
async function getRedditMetadata(postId: string): Promise<Metadata> {
  try {
