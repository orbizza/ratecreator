...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return new NextResponse(
      renderHtmlPage(
        "Unsubscribed",
        "You have been successfully unsubscribed from the Creator Portal newsletter.",
      ),
      { status: 200, headers: { "Content-Type": "text/html" } },
    );
  } catch (error) {
...
<head>
  ...
  <title>${title} - Creator Portal</title>
  <style>
    body {
...
<body>
  <div class="container">
    <h1>${title}</h1>
    <p>${message}</p>
    <a href="https://creator.ratecreator.com">Go to Creator Portal</a>
  </div>
</body>
</html>`;
}
